...
def decode_outputs(outs, ensembles_targets, cuda=True, coder=None):
        ...
        N_PC = pc_targets.shape[-1]
        N_HDC = hd_targets.shape[-1]
        ...
        logits_pc = logits_pc.view(-1, N_PC)
        logits_hd = logits_hd.view(-1, N_HDC)

        if coder:
            pc_targets, hd_targets = coder(pc_targets, target=True), coder(hd_targets, target=True)

        pc_targets = pc_targets.contiguous().view(-1, N_PC)
        hd_targets = hd_targets.contiguous().view(-1, N_HDC)
