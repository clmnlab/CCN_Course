...
def softmax(x, axis=-1):
    ...
    return nn.functional.softmax(x, dim=axis)
...
class CellEnsemble:
    ...
    def get_targets(self, x):
        """Type of target."""
        targets = torch.empty_like(x)  ## added by SK
    ...
    def get_init(self, x):
        ...
        init = torch.empty_like(x)  ## added by SK
    ...
    def unnor_logpdf(self, x):  ## added by SK
        unnor_logp = -0.5 * torch.sum((x ** 2) / 10, dim=-1)
        return unnor_logp
