...
TARGET_ACTION = 0  # Target action (0: Left, 1: Right)
ALT_ACTION = 1 # Non-target action
# TARGET_ACTION = 0
# ALT_ACTION = 1
# QLearner Hyperparameters
LEARNER_ALPHA = 0.1
LEARNER_GAMMA = 0.9
LEARNER_EPSILON = 0.1
...
class AdversaryAgent:
    ...
    def load_adversary(self, path="adversary_policy.pth"):
        ...
        try:
            ...
            # loading policy_net and target_net 
    ...
    def train(self, is_training=True):        
        ...
        for i_episode in tqdm(range(self.n_episodes), desc="Adversary Training"):
            ...
            total_adv_rewards = 0
            total_adv_actions = []
            adv_state = self.reset()
            for t in range(self.n_trials):
                # 1. adversary action: assigning rewards
                adv_action = self.select_action(adv_state, is_training=is_training)
                # print(adv_action)
                adv_action_tensor = torch.tensor([[adv_action]], device=device, dtype=torch.long)
                # 2. learner selects action
                learner_action, action_probs = self.learner.select_action()
                # 3. reward to learner based on adversary's assignment
                reward_to_learner = self.reward_assignment[learner_action]
                p_target = action_probs[TARGET_ACTION]
                # 4. learner updates its Q-values
                self.learner.update(reward_to_learner)
                # 5. reward to adversary: did learner choose TARGET_ACTION?
                adv_reward = 1 if learner_action == TARGET_ACTION else 0
                # print(adv_reward)
                total_adv_rewards += adv_reward 
                adv_reward_tensor = torch.tensor([adv_reward], device=device)
                # 5. record experience
                current_episode_experience.append([adv_action, learner_action, p_target, reward_to_learner, adv_reward])
                next_learner_state = self.learner.get_observable_state()
                adv_next_state = None
                if t < TOTAL_TRIALS - 1:
                ...
                self.memory.push(adv_state, adv_action_tensor, adv_reward_tensor, next_state)
                ## Important, don't miss this line to update state
                self.reward_assignment = [0, 0]  # Reset for next trial (Really important!!)
        ...
        return total_adv_rewards_all, losses, total_adv_actions_all, current_episode_experience
